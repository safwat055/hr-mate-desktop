            ...
            CompletableFuture.runAsync(() -> {
                try {
                    if (file != null && file.exists()) {
                        ...
                        Platform.runLater(() ->
                                SAFNotification.error("الملف غير موجود"));
                    }
                } catch (IOException ex) {
                    Platform.runLater(() ->
                            SAFNotification.error("لا يمكن فتح الملف: " + ex.getMessage()));
                }
            });
        });
            ...
            if (file != null && file.exists()) {
                CompletableFuture.runAsync(() -> {
                    try {
                        ...
                        Platform.runLater(() ->
                                SAFNotification.error("لا يمكن فتح المجلد: " + ex.getMessage()));
                    }
                });
            } else {
                SAFNotification.error("المجلد غير موجود");
            }
        });
